...
class Demo {
  ...
  private void run()
    ...
    System.out.printf("Async event [{}] has been created.%n", asyncEventId);
    eventManager.start(asyncEventId);
    System.out.printf("Async event [{}] has been started.%n", asyncEventId);

    // Create a Synchronous event.
    final int syncEventId = eventManager.createSyncEvent(60);
    System.out.printf("Sync event [{}] has been created.%n", syncEventId);
    eventManager.start(syncEventId);
    System.out.printf("Sync event [{}] has been started.%n", syncEventId);
