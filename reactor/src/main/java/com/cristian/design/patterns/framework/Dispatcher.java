...
/**
 * Represents the event dispatching strategy. When {@link NioReactor} senses any event on the registered {@link AbstractNioChannel}s then it
 * de-multiplexes the event type, read or write or connect, and then calls the {@link Dispatcher} to dispatch the read events. This
 * decouples the I/O processing from application specific processing. <br> Dispatcher should call the {@link ChannelHandler} associated with
 * the channel on which event occurred.
 *
...
public interface Dispatcher {
  ...
  /**
   * This hook method is called when read event occurs on particular channel. The data read is provided in <code>readObject</code>. The
   * implementation should dispatch this read event to the associated {@link ChannelHandler} of <code>channel</code>.
   *
