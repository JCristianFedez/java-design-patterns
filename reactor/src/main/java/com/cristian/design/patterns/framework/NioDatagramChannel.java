...
public class NioDatagramChannel extends AbstractNioChannel {
  ...
  /**
   * Creates a {@link DatagramChannel} which will bind at provided port and use <code>handler</code> to handle incoming events on this
   * channel.
   *
  ...
  /**
   * Writes the outgoing {@link DatagramPacket} to the channel. The intended receiver of the datagram packet must be set in the
   * <code>data</code> using {@link DatagramPacket#setReceiver(SocketAddress)}.
   */
  @Override
  ...
  public void bind() throws IOException {
    getChannel().socket().bind(new InetSocketAddress(InetAddress.getLocalHost(), this.port));
    getChannel().configureBlocking(false);
    LOGGER.info("Bound UDP socket at port: {}", this.port);
  }

  @Override
  public Object read(final SelectionKey key) throws IOException {
  ...
  /**
   * Writes the pending {@link DatagramPacket} to the underlying channel sending data to the intended receiver of the packet.
   */
  @Override
